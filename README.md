...
> **项目性质**：本科毕业设计项目  
> **项目定位**：基于「规则引擎 + LLM」双轮驱动的合同风险审查智能体原型系统  
> **当前版本**：v2.2 (2025-12-31 更新)
...
本项目旨在解决传统人工合同审查中存在的效率低、易漏判、标准不一等问题。通过构建一个**垂直领域的法律智能体 (Legal Agent)**，实现对商业合同的自动化风险监测。

系统采用 **"Rule-Guided + LLM-Reasoning"** (规则引导 + 大模型推理) 的混合架构：
1.  **规则引擎**作为"守门员"，利用确定性的专家知识快速识别已知风险点，并召回精准的法律依据。
2.  **大语言模型**作为"分析师"，在规则和法条的约束下进行逻辑推理，识别复杂的隐蔽风险，并生成人性化的修改建议。

> ⚠️ **免责声明**：本项目为学术研究与技术验证性质，仅供学习交流，不构成任何法律建议。在签署重要合同时，请务必咨询专业律师。

---

## 🚀 v2.0 架构重构 (Migration to Reflex)

为了提供更现代化的交互体验和更稳定的系统架构，本项目在 v2.0 版本中进行了从 **Streamlit** 到 **Reflex** 的全面重构。

### 核心改进
*   **交互体验升级 (UX Overhaul)**
    *   **响应式玻璃拟态 UI**: 采用 Glassmorphism 设计语言，提供沉浸式的视觉体验。
    *   **双面板即时交互**: 首页采用 Split-View 设计，左侧操作与右侧预览实时同步，无需频繁跳转。
    *   **无缝工作流**: 支持文件拖拽自动上传解析，去除了冗余的确认步骤。

*   **系统架构解耦 (Architecture Decoupling)**
    *   **Frontend**: 使用 **Reflex** (基于 React) 构建高性能前端，状态管理更加可控。
    *   **Backend**: 独立封装 **FastAPI** 服务层，负责处理耗时的 LLM 分析与检索任务，通过异步接口与前端通信，彻底解决了界面卡顿问题。

*   **智能能力增强 (Capability Enhancement)**
    *   **三级风险评估体系**: 支持 🔴高/🟠中/🟢低 三级风险分类，基于合法性、经济性、确定性三维评估。
    *   **深度反思模式 (Self-Reflection)**: 对高/中风险结果触发二次审查，支持「维持/调级/存疑」三种结论，调级逻辑支持双向调整。
    *   **Top-K 规则匹配**: 支持召回多个候选规则（可配置 top_k），为 LLM 提供更丰富的专家上下文。
...
*   **双路召回**:
    *   **Dense Path**: 使用 `BGE-small-zh-v1.5` 生成高维语义向量，捕获"语义相似"的风险规则。
    *   **Sparse Path**: 使用 `BM25Okapi` + `jieba` 分词，捕获"关键词精确匹配"的风险规则。
*   **加权融合**: 采用 `α × Dense + (1-α) × Sparse` 加权求和算法融合两路结果（默认 α=0.5），有效解决幻觉问题。
*   **Top-K 多规则召回**: 支持返回多个超过置信度阈值的候选规则（默认 top_k=3），提供更丰富的上下文。

### 4. 📚 法律与专家知识库
*   **风险规则库**: 内置 120+ 条经过法律专家标注的常见商业合同风险点（如：霸王条款、不合理违约金、管辖权陷阱）。
...
*   **Context-Aware Prompting**: 将召回的规则、法条、条款上下文动态注入 Prompt，约束 LLM 的生成范围。
*   **Self-Reflection (深度反思)**: 对高/中风险触发二次审查，支持「维持/调级/存疑」三种结论。

### 7. 📊 交互式风险报告
*   **结构化输出**: 生成 Markdown 格式的详细报告，包含风险等级、风险点摘要、法条依据、修改建议。
*   **可视化大屏**: 提供风险概览仪表盘，支持按高/中/低风险等级筛选查看。
*   **Word 导出**: 一键导出专业 Word 格式报告，支持三级风险颜色标识（红/橙/绿）。
...
| **向量模型** | **BGE / SBERT** | 中文语义向量表征 (Sentence-Transformers) |
| **全文检索** | **Rank-BM25** | 传统的词袋模型检索，保证关键词命中率 |
| **数据存储** | **SQLite** | 轻量级关系型数据库，存储法条与历史记录 |

---

## 🧪 消融实验评测系统 (Ablation Study)

系统内置了完整的消融实验框架，用于量化各组件对最终效果的贡献。
...
### 4. 启动系统
使用项目根目录下的启动脚本，一键拉起后端 API 和前端界面：
